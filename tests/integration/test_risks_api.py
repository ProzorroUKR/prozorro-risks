...
async def test_list_tenders_sort_by_date_modified(api, db):
    ...
    tender_with_3_1_risk_found["dateModified"] = "2019-03-14T21:37:16.832566+02:00"
    ...
    assert resp_json["items"][-1]["dateModified"] == tender_with_3_1_risk_found["dateModified"]

    response = await api.get("/api/risks?order=desc")
    assert response.status == 200
    resp_json = await response.json()
    assert resp_json["items"][0]["dateModified"] == tender_with_3_1_risk_found["dateModified"]
...
async def test_list_tenders_filter_by_risks_worked(api, db):
    ...
    assert resp_json["items"][0]["risks"]["worked"][-1]["id"] in ("3-1", "3-2")
