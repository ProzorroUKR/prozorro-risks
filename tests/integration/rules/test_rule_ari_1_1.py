...
from prozorro.risks.rules.ari_1_1 import RiskRule
...
async def test_tender_with_two_active_changes_in_contract():
    contract = deepcopy(contract_data)
    contract["changes"][2]["status"] = "pending"
    risk_rule = RiskRule()
    result = await risk_rule.process_contract(contract, tender_data)
...
async def test_tender_with_three_active_changes_in_contract():
    risk_rule = RiskRule()
