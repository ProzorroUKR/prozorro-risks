...
def _build_sas24_3_5_violation_tender(date_created, winner_date):
...
async def test_winner_zero_days_old_still_triggers():
    tender = _build_sas24_3_5_violation_tender(get_now().isoformat(), get_now().isoformat())
    risk_rule = RiskRule()
    result = await risk_rule.process_tender(tender)
    assert result == RiskFound()
...
async def test_tender_older_than_180_days_returns_no_risk():
    tender = _build_sas24_3_5_violation_tender(
