...
from prozorro.risks.rules.ari_1_2 import RiskRule
...
async def test_tender_with_less_than_two_quality_changes_in_contract():
    contract = deepcopy(contract_data)
    contract["changes"][0]["rationaleTypes"] = ["qualityImprovement"]
    contract["changes"][1]["rationaleTypes"] = ["durationExtension", "fiscalYearExtension"]  # not risky rationale
    contract["changes"][2]["rationaleTypes"] = ["fiscalYearExtension"]  # not risky rationale
    risk_rule = RiskRule()
    result = await risk_rule.process_contract(contract, tender_data)
    assert result == RiskNotFound(type="contract", id=contract["id"])
...
async def test_tender_with_three_quality_changes_in_contract():
    contract = deepcopy(contract_data)
    contract["changes"][0]["rationaleTypes"] = ["qualityImprovement"]
    contract["changes"][1]["rationaleTypes"] = ["qualityImprovement", "fiscalYearExtension"]
    contract["changes"][2]["rationaleTypes"] = ["qualityImprovement"]
    risk_rule = RiskRule()
    result = await risk_rule.process_contract(contract, tender_data)
    assert result == RiskFound(type="contract", id=contract["id"])


async def test_tender_with_not_all_quality_active_changes():
    # contract_data has 2 changes which contain qualityImprovement in rationaleTypes array but not all active
    contract = deepcopy(contract_data)
    contract["changes"][0]["rationaleTypes"] = ["qualityImprovement"]
    contract["changes"][1]["rationaleTypes"] = ["qualityImprovement", "fiscalYearExtension"]
    contract["changes"][1]["status"] = ["pending"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("priceQuotation", RiskFound(type="contract", id=contract_data["id"])),
    ],
)
